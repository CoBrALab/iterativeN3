...
cleanup_classify() {
  EXTRACTION_SEGMENTATION=${1}
  EXTRACTION_SEGMENTATION2=${2}
  EXTRACTION_WM=${tmpdir}/${n}/3.mnc
  EXTRACTION_GM=${tmpdir}/${n}/2.mnc
  EXTRACTION_CSF=${tmpdir}/${n}/1.mnc
  EXTRACTION_DEEPGM=${tmpdir}/${n}/4.mnc

  EXTRACTION_MASK=${3}

  #Extract Classes
  ThresholdImage 3 ${EXTRACTION_SEGMENTATION} ${EXTRACTION_WM} 3 3 1 0
  ThresholdImage 3 ${EXTRACTION_SEGMENTATION} ${EXTRACTION_GM} 2 2 1 0
  ThresholdImage 3 ${EXTRACTION_SEGMENTATION} ${EXTRACTION_CSF} 1 1 1 0


  #Remove outlier voxels
  LabelClustersUniquely 3 ${EXTRACTION_WM} ${EXTRACTION_WM} 50
  LabelClustersUniquely 3 ${EXTRACTION_GM} ${EXTRACTION_GM} 50


  ThresholdImage 3 ${EXTRACTION_WM} ${EXTRACTION_WM} 1 Inf 1 0
  ThresholdImage 3 ${EXTRACTION_GM} ${EXTRACTION_GM} 1 Inf 1 0


  #Fill in holes
  ImageMath 3 ${EXTRACTION_WM} FillHoles ${EXTRACTION_WM} 2
  ImageMath 3 ${EXTRACTION_GM} FillHoles ${EXTRACTION_GM} 2

  #Re-form tissue classes
  ImageMath 3 ${EXTRACTION_WM} m ${EXTRACTION_WM} 3
  ImageMath 3 ${EXTRACTION_GM} m ${EXTRACTION_GM} 2
  ImageMath 3 ${EXTRACTION_SEGMENTATION2} addtozero ${EXTRACTION_GM} ${EXTRACTION_WM}

  if [[ ! -z ${DEEPGMPRIOR} ]]; then
    ThresholdImage 3 ${EXTRACTION_SEGMENTATION} ${EXTRACTION_DEEPGM} 4 4 1 0
    LabelClustersUniquely 3 ${EXTRACTION_DEEPGM} ${EXTRACTION_DEEPGM} 50
    ThresholdImage 3 ${EXTRACTION_DEEPGM} ${EXTRACTION_DEEPGM} 1 Inf 1 0
    ImageMath 3 ${EXTRACTION_DEEPGM} FillHoles ${EXTRACTION_DEEPGM} 2
    ImageMath 3 ${EXTRACTION_DEEPGM} m ${EXTRACTION_DEEPGM} 4
    ImageMath 3 ${EXTRACTION_SEGMENTATION2} addtozero ${EXTRACTION_SEGMENTATION2} ${EXTRACTION_DEEPGM}
  fi

  ThresholdImage 3 ${EXTRACTION_SEGMENTATION2} ${EXTRACTION_MASK} 2 Inf 1 0
  ImageMath 3 ${EXTRACTION_MASK} FillHoles ${EXTRACTION_MASK} 2

  ImageMath 3 ${EXTRACTION_SEGMENTATION2} PropagateLabelsThroughMask ${EXTRACTION_MASK} ${EXTRACTION_SEGMENTATION2} 200000 1

  iMath 3 ${EXTRACTION_MASK} MC ${EXTRACTION_MASK} 5 1 ball 1
  iMath 3 ${EXTRACTION_MASK} MD ${EXTRACTION_MASK} 1 1 ball 1
  ImageMath 3 ${EXTRACTION_MASK} FillHoles ${EXTRACTION_MASK} 2
  ImageMath 3 ${EXTRACTION_SEGMENTATION2} addtozero ${EXTRACTION_SEGMENTATION2} ${EXTRACTION_MASK}

  ImageMath 3 ${EXTRACTION_SEGMENTATION2} m ${EXTRACTION_SEGMENTATION2} ${EXTRACTION_MASK}

  if [[ ! -z ${DEEPGMPRIOR} ]]; then
    ThresholdImage 3 ${tmpdir}/${n}/posterior4.mnc ${tmpdir}/${n}/4.mnc 0.05 Inf 1 0
    ImageMath 3 ${tmpdir}/${n}/4.mnc GetLargestComponent ${tmpdir}/${n}/4.mnc
    ImageMath 3 ${tmpdir}/${n}/posterior4.mnc m ${tmpdir}/${n}/posterior4.mnc ${tmpdir}/${n}/4.mnc
  fi

  #for j in 1 2 3 4; do
  #  ThresholdImage 3 ${EXTRACTION_SEGMENTATION2} ${tmpdir}/${n}/${j}.mnc ${j} ${j} 1 0
  #  ImageMath 3 ${tmpdir}/${n}/posterior${j}.mnc m ${tmpdir}/${n}/posterior${j}.mnc ${tmpdir}/${n}/${j}.mnc
  #done
}
